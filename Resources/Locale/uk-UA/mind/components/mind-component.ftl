...
comp-mind-examined-catatonic = { CAPITALIZE(SUBJECT($ent)) } { CONJUGATE-BE($ent) } знаходиться у стані глибокої кататонії. Мабуть, навантаження життя у глибокому космосі були для { OBJECT($ent) } занадто важкими. Ймовірність одужання малоймовірна.
comp-mind-examined-dead = { CAPITALIZE(SUBJECT($ent)) } { CONJUGATE-BE($ent) } виглядає мертво.
comp-mind-examined-ssd = { CAPITALIZE(SUBJECT($ent)) } { CONJUGATE-HAVE($ent) } порожній, розсіяний погляд і, здається, ні на що не реагує. Можливо { SUBJECT($ent) } скоро роздуплиться.
