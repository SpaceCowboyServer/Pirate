...
public sealed class PublicTransitSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        Enabled = _cfgManager.GetCVar(CCVars.PublicTransit);
        FlyTime = _cfgManager.GetCVar(CCVars.PublicTransitFlyTime);
        Counter = 0;
        StationList.Clear();
        _cfgManager.OnValueChanged(CCVars.PublicTransit, SetTransit);
        _cfgManager.OnValueChanged(CCVars.PublicTransitFlyTime, SetFly);
    }
    ...
    {
        base.Shutdown();
        _cfgManager.UnsubValueChanged(CCVars.PublicTransitFlyTime, SetFly);
        _cfgManager.UnsubValueChanged(CCVars.PublicTransit, SetTransit);
    }
        ...
        while (consoleQuery.MoveNext(out var consoleUid, out _))
        {
            if (Transform(consoleUid).GridUid == uid)
            {
                ...
                _chat.TrySendInGameICMessage(consoleUid, Loc.GetString("public-transit-arrival",
                        ("destination", destinationString), ("waittime", _cfgManager.GetCVar(CCVars.PublicTransitWaitTime))),
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var comp, out var shuttle))
        {
            ...
            comp.NextTransfer += TimeSpan.FromSeconds(FlyTime + _cfgManager.GetCVar(CCVars.PublicTransitWaitTime));
        }
    }
    ...
    private void SetupPublicTransit()
    {
        ...
        var busMap = _cfgManager.GetCVar(CCVars.PublicTransitBusMap);
        if (_loader.TryLoad(dummyMap, busMap, out var shuttleUids))
        {
            ...
            if (TryGetNextStation(out var station) && station is { Valid : true } destination)
            {
                ...
                transitComp.NextTransfer = _timing.CurTime + TimeSpan.FromSeconds(_cfgManager.GetCVar(CCVars.PublicTransitWaitTime));
